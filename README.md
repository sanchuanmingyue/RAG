...
工作流。Chat 中可通过“多模态”开关按需切换到 RAG-Anything；单篇论文可独立开启“深度思考”，
多篇论文范围则自动启用完整检索与证据规划。流式问答会显示“检索中、重排中、生成中”的当前阶段。
...
检索/生成耗时。普通单篇问答默认使用 Top-5 和 `QA_MAX_TOKENS=1200`；开启“深度思考”后，先生成内部
证据提纲，再使用 Top-12、最多 40000 字符上下文和 `QA_LONG_MAX_TOKENS=3600` 完成写作。选择全部论文时
会强制启用完整规划并默认检索 Top-16。深度检索的多个互补查询默认由 `QA_RETRIEVAL_WORKERS=4` 并发执行，
结果仍按固定查询顺序合并；设为 1 可恢复串行执行。如果模型因长度上限停止，
